...
-- current modifier status, so the keybindings will work with numlock on op | grep Num
...
myKeys conf@(XConfig {XMonad.modMask = modm}) = M.fromList $
    ...
    -- launch dmenu
    , ((modm,               xK_p     ), spawn "exe=`dmenu_path | dmenu -i -nb black -nf red -sf green` && eval \"exec $exe\"")
...
-- imLayout = ThreeCol 2 (1/50) (1/6) ||| ThreeColMid 2 (1/50) (1/6) ||| Full 
...
myManageHook = (composeAll . concat $
    ...
    , [className    =? c          --> doShift  "4:IM"      |   c   <- myChat   ]      -- move chat to chat
...
myStartupHook = do spawn "xset r rate 400 75"
